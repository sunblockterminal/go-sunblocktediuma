...
{{end}}
RUN \
  echo 'gsbt --cache 512 init /genesis.json' > gsbt.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.sunblocktedium/keystore/ && cp /signer.json /root/.sunblocktedium/keystore/' >> gsbt.sh && \{{end}}
	echo $'exec gsbt --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gsbt.sh

ENTRYPOINT ["/bin/sh", "gsbt.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.sunblocktedium{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.sbthash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.sunblocktedium"],
		ethashdir:  infos.volumes["/root/.sbthash"],
		port:       port,
