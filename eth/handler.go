...
import (
	...
	"github.com/sunblockterminal/go-sunblocktediuma/eth/fetcher"
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Sunblocktedium peer", "peer", id)
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Sunblocktedium protocol")
	...
	log.Info("Sunblocktedium protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Sunblocktedium peer connected", "name", p.Name())
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Sunblocktedium handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Sunblocktedium peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Sunblocktedium message handling failed", "err", err)
			return err
		}
	}
}
